...
use rls::server::{self as ls_server, Notification, Request, RequestId};
use jsonrpc_core;
...
use self::support::{rls_timeout, fixtures_dir};
use serde_json::json;
...
fn location(
    ...
    let (path, pat) = (path.as_ref(), pat.as_ref());
    let file = std::fs::File::open(path)
        .unwrap_or_else(|_| panic!("Couldn't open file {}", path.display()));

    let line = BufReader::new(&file).lines().nth((line_1_based - 1) as usize)
        .unwrap_or_else(|| panic!("Couldn't fetch line {}", line_1_based))
        .unwrap();

    let byte_offset = line.find(pat)
        .unwrap_or_else(|| panic!("Couldn't find {} at line {}", pat, line_1_based));
    let byte_len: usize = pat.chars().map(|c| c.len_utf16()).sum();

    languageserver_types::Location {
        uri: url::Url::from_file_path(path).unwrap(),
        range: languageserver_types::Range {
            start: languageserver_types::Position {
                line: (line_1_based - 1).into(),
                character: byte_offset as u64,
            },
            end: languageserver_types::Position {
                line: (line_1_based - 1).into(),
                character: (byte_offset + byte_len) as u64,
            },
        }
    }
}
...
fn test_goto_def_translated() {
    ...
    rls.request_typed::<requests::Definition>(
        11,
        TextDocumentPositionParams {
            text_document: TextDocumentIdentifier::new(p.file_uri("src/main.rs")),
            position: position(p.file_path("src/main.rs"), 22, "world"),
        },
    )
    .unwrap();

    let json = rls.wait_until_json_id(11, rls_timeout());

    let val: languageserver_types::Location =
        serde_json::from_value(json["result"][0].clone()).unwrap();
    assert_eq!(val, location(p.file_path("src/main.rs"), 21, "world"));
