...
impl RlsHandle {
    ...
    pub fn wait_for_indexing(&mut self) {
        self.take_messages_until(|msg| {
            msg.get("params")
                .map(|params| {
                    params.get("title").and_then(|x| x.as_str()) == Some("Indexing")
                        && params.get("done").and_then(|x| x.as_bool()) == Some(true)
                })
                .unwrap_or(false)
        });
    }
    ...
    pub fn wait_for_id(&mut self, id: u64) {
        self.take_messages_until(|msg| msg.get("id").and_then(|x| x.as_u64()) == Some(id));
    }
